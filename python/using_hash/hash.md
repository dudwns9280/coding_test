...
### 문제 풀이
1) 장르 별 총 재생 횟수로 장르 별 순서 결정.
2) 각 장르에서 각 노래들의 순위 결정.(최대 2순위까지)
3) 장르 순서대로 각 잘르의 노래 순위를 출력하면 끝!

실제 과정  
genre 기반 for문 진행.  
genre_count_dict라는 장르별 재생 횟수 dict 생성 및 누적.  
genre_dict라는 장르에 속한 노래의 고유 번호 리스트를 가진 dict 생성 및 쌓기  
play_dict라는 노래의 고유번호를 Key값, 재생 횟수는 value로 가진 dict 생성 및 쌓기  

genre_count_dict sorted(,key=lambda x:x[1], reverse=Ture)를 통해  
총 재생 횟수가 가장 많은 장르 부터 불러옴 (장르의 개수는 최대 100개까지만 허용)  
genre 별 노래 list를 가지고 해당 장르의 노래 고유번호와 해당 노래의 재생 횟수들을 dict형식으로 생성  
해당 dict를 재생 횟수 별로 sorted하여 앞의 key 2개 꺼내서 answer에 누적 후 다음 장르로 넘어감.  
해당 과정 반복하면 끝!  